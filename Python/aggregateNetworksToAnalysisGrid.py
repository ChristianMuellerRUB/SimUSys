# -*- coding: utf-8 -*-
# --------------------------------------------------------------------
# Name: aggregateNetworksToAnalysisGrid.py
# --------------------------------------------------------------------
...
if (os.path.isfile(os.path.join(modelFolder, "highestResAttributes_Net.csv")) == False):
    rScript = os.path.join(rScriptPath, "getNetworkEntitiesWithHighestResolution.r")
...
userTab = []
with open(os.path.join(modelFolder, "highestResAttributes_Net.csv"), 'r') as csvfile:
...
for thisPlanEnt in range(1, len(userTab)):
    ...
    if fromGeometry != "Point":
        ...
        if (areaField not in namesFrom):
            thisString = areaField + "Fr"
            # thisString = thisString[0:9]
            arcpy.AddField_management(fromShp, thisString, "Double")
            if fromGeometry == "Polygon":
                arcpy.CalculateField_management(fromShp, thisString, "!shape.area@squaremeters!", "PYTHON")
            if fromGeometry == "Line":
                arcpy.CalculateField_management(fromShp, thisString, "!shape.length@meters!", "PYTHON")
